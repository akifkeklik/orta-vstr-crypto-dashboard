...
const getTrendStatus = (v) => {
    const n = Number(v) || 0;
    if (n > 0) return { color: 'text-emerald-500', hex: '#10b981', sign: '+', icon: TrendingUp, bg: 'bg-emerald-500' };
    if (n < 0) return { color: 'text-rose-500', hex: '#f43f5e', sign: '', icon: TrendingDown, bg: 'bg-rose-500' };
    return { color: 'text-amber-400', hex: '#fbbf24', sign: '', icon: Minus, bg: 'bg-amber-400' };
};

const formatCurrency = (v) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 }).format(v);
...
const smartTickFormatter = (ts, periodLabel) => {
    ...
    if (label.includes('1 saat') || label.includes('24 saat')) return date.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
    return date.toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' });
};

// SABİT 6 KART KURALI
const FIXED_INTERVALS = ['1 Saat', '24 Saat', '7 Gün', '30 Gün', '6 Ay', '1 Yıl'];
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const loadHist = async () => {
      ...
      const startTime = new Date(Date.now() - getPeriodMs(selectedCoin.zaman_araligii)).toISOString();
      
      const { data: hist } = await supabase.from('kripto_analiz')
        .select('created_at, fiyat, coin_adi')
        .gte('created_at', startTime)
        .order('created_at', { ascending: true });

      if (hist) {
          const filteredHist = hist.filter(h => (h.coin_adi || 'ORTA').toUpperCase() === selectedCoin.coin_adi);
        ...
        <div className={`h-24 flex items-center px-6 border-b ${theme.border} ${isSidebarOpen ? 'justify-between' : 'justify-center'}`}>
          {isSidebarOpen ? (
            <div className="flex flex-col">
              <span className="font-bold text-lg tracking-tight uppercase">VSTR ECOSYSTEM</span>
              <span className="text-[9px] text-emerald-500 font-mono tracking-widest uppercase opacity-60">Control Panel</span>
            </div>
          ) : <VstrLogo className="w-8 h-8 text-emerald-500" />}
        </div>
        <nav className="flex-1 py-8 px-3 space-y-2">
          <div onClick={() => setActiveTab('VSTR')} className={`flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer group transition-all ${activeTab === 'VSTR' ? 'bg-emerald-500/10 text-emerald-500' : 'opacity-50 hover:opacity-100'}`}>
            <VstrLogo className="w-5 h-5 flex-shrink-0" />
            {isSidebarOpen && <span className="text-sm font-semibold uppercase tracking-tighter">VSTR</span>}
          </div>
          <div onClick={() => setActiveTab('ORTA')} className={`flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer group transition-all ${activeTab === 'ORTA' ? 'bg-emerald-500/10 text-emerald-500' : 'opacity-50 hover:opacity-100'}`}>
            <OrtaLogo className="w-5 h-5 flex-shrink-0" />
            {isSidebarOpen && <span className="text-sm font-semibold uppercase tracking-tighter">ORTA</span>}
          </div>
        </nav>
        <div className="p-3">
          <button onClick={() => setDarkMode(!darkMode)} className={`flex items-center gap-3 p-3 rounded-xl ${theme.inputBg} w-full ${!isSidebarOpen && 'justify-center'}`}>
            {darkMode ? <Sun size={18}/> : <Moon size={18}/>}
            {isSidebarOpen && <span className="text-xs font-bold uppercase tracking-wider">MOD</span>}
          </button>
        </div>
      </aside>
                   ...
                   chartHistory.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={chartHistory}>
                            <defs><linearGradient id="c" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={getTrendStatus(selectedCoin.degisim_yuzdesi).hex} stopOpacity={0.2}/><stop offset="95%" stopColor={getTrendStatus(selectedCoin.degisim_yuzdesi).hex} stopOpacity={0}/></linearGradient></defs>
                            <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? "#ffffff05" : "#00000010"} vertical={false} />
                            <XAxis dataKey="ts" type="number" domain={[Date.now() - getPeriodMs(selectedCoin.zaman_araligii), Date.now()]} tickFormatter={(t) => smartTickFormatter(t, selectedCoin.zaman_araligii)} fontSize={10} stroke="#525252" tickLine={false} axisLine={false} />
                            <YAxis domain={([dataMin, dataMax]) => {
                                    const diff = dataMax - dataMin;
                                    const margin = diff === 0 ? dataMin * 0.1 : diff * 0.4;
                                    return [dataMin - margin, dataMax + margin];
                                }} hide />
                            <Tooltip contentStyle={{ backgroundColor: '#000', border: '1px solid #222', borderRadius: '12px', fontSize: '12px' }} labelFormatter={t => new Date(t).toLocaleString('tr-TR')} />
                            <Area type="monotone" dataKey="val" stroke={getTrendStatus(selectedCoin.degisim_yuzdesi).hex} strokeWidth={3} fill="url(#c)" animationDuration={1000} />
                        </AreaChart>
                    </ResponsiveContainer>
                   ) : <div className="h-full flex flex-col items-center justify-center opacity-10 gap-4"><Layers size={60} /><p className="font-mono text-xs font-black uppercase tracking-widest">Syncing...</p></div>}
               </div>
            </div>
          </div>
        </div>
